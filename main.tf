...
resource "azurerm_public_ip" "pip" {
  ...
  name                = lower("pip-vm-${var.vmscaleset_name}-${var.location}-0${count.index + 1}")
  ...
  tags                = merge({ "resourcename" = lower("pip-vm-${var.vmscaleset_name}-${var.location}-0${count.index + 1}") }, var.tags, )
...
resource "azurerm_lb" "vmsslb" {
  ...
  name                = var.load_balancer_type == "public" ? lower("lbext-${var.vmscaleset_name}-${var.location}") : lower("lbint-${var.vmscaleset_name}-${var.location}")
  location            = var.location
  resource_group_name = var.resource_group_name
  sku                 = var.load_balancer_sku
  tags                = merge({ "resourcename" = var.load_balancer_type == "public" ? lower("lbext-${var.vmscaleset_name}-${var.location}") : lower("lbint-${var.vmscaleset_name}-${var.location}") }, var.tags, )
...
resource "azurerm_proximity_placement_group" "appgrp" {
  ...
  name                = lower("proxigrp-${var.vmscaleset_name}-${var.location}")
  resource_group_name = var.resource_group_name
  location            = var.location
  tags                = merge({ "resourcename" = lower("proxigrp-${var.vmscaleset_name}-${var.location}") }, var.tags, )
...
#   name                = lower("nsg_${var.vmscaleset_name}_${var.location}_in")
#   resource_group_name = var.resource_group_name
#   location            = var.location
#   tags                = merge({ "resourcename" = lower("nsg_${var.vmscaleset_name}_${var.location}_in") }, var.tags, )
...
#   destination_address_prefix  = var.address_prefixes #(concat(data.azurerm_subnet.snet.address_prefixes, [""]), 0)
...
resource "azurerm_linux_virtual_machine_scale_set" "linux_vmss" {
  ...
  dynamic "admin_ssh_key" {
    ...
    content {
      public_key = one(module.ssh_key_pair).public_key
  ...
  network_interface {
    ...
    ip_configuration {
      ...
      application_gateway_backend_address_pool_ids = var.enable_load_balancer != true ? var.application_gateway_backend_address_pool_ids : null
...
resource "azurerm_windows_virtual_machine_scale_set" "winsrv_vmss" {
  ...
  instances                                         = var.instances_count
  admin_username                                    = var.admin_username
  admin_password                                    = var.admin_password == null ? one(module.win_password).password : var.admin_password
  custom_data                                       = var.custom_data
  overprovision                                     = var.overprovision
  do_not_run_extensions_on_overprovisioned_machines = var.do_not_run_extensions_on_overprovisioned_machines
